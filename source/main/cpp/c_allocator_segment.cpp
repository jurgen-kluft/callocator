...
namespace ncore
{
    ...
    // number of bits per size is 64 * 64 * 64 = 262144 bits = 256 K bits.
    ...
    inline u64 clr_bit(segment_alloc_t* sa, s8 size_index, s32 bit)
    {
    ...
    s32 find_and_clr_bit(segment_alloc_t* sa, s8 size_index)
    {
        const s32 bit = find_bit(sa, size_index);
        if (bit >= 0 && clr_bit(sa, size_index, bit) == 0)
        {
            sa->m_size_free &= ~(1 << size_index); // No more free elements at this size
        }
        return bit;
    }
    ...
    s8 set_bit(segment_alloc_t* sa, s8 size_index, s32 bit)
    {
        ...
        {
            if (clr_bit(sa, size_index, bit ^ 1) == 0)
                return 0;
            return 2;
        }
    ...
    bool segment_allocate(segment_alloc_t* sa, s64 size, s64& offset)
    {
        ...
        const u32 size_free = (sa->m_size_free & ~((1 << size_index) - 1));
        if (size_free == 0)
        {
            // Fragmented / OOM ?
            offset = -1;
        ...
        offset = ((s64)1 << (sa->m_min_size_shift + size_index)) * bit;
        return true;
    }
    ...
    bool segment_deallocate(segment_alloc_t* sa, s64 ptr, s64 size)
    {
        ...
        while (size_index >= 0)
        {
            const s8 result = set_bit(sa, size_index, bit);
            if (result == 1)
                break;
            if (result == 0)
                sa->m_size_free &= ~(1 << size_index);

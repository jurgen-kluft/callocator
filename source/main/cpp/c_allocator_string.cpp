    ...
    namespace nstring
    {
        namespace nutf8
        {
            ...
            public:
                ...
                static s8 s_compare_str(item_t const* strA, item_t const* strB)
                {
                ...
                static s8 s_compare_str_to_node(u32 const _str, u32 const _node, void const* user_data)
                {
                    ...
                    item_t const*    str      = members->m_items + _str;
                    item_t const*    node_str = members->m_items + _node;
                    return s_compare_str(str, node_str);
                }

                void setup(void* mem, size_t mem_size, u32 max_items)
                {
                    m_data.m_items = (item_t*)mem;
                ...
                id_t v_put(crunes_t const& str) override
                {
                    ...
                    if (!ntree32::find(m_data.m_tree, m_data.m_root, key_node, s_compare_str_to_node, this, found_node))
                    {
                        ...
                        item_t* insert_item = m_data.m_items + found_node;
                        *insert_item        = *item;
                        m_data.m_size++;
                        m_data.m_str_cursor = dst8 + 1; // +1 to include the terminator.
                    }
                    return (id_t)found_node;
                }

                crunes_t v_get(id_t ptr) const override
                {
                    item_t* item = m_data.m_items + ptr;
                    return make_crunes(item->m_str, 0, item->m_len, item->m_len);
                }
            };
            ...
            static u32 compute_max_items(size_t memory_size)
            {
                ...
                return (u32)((memory_size - sizeof(utf8_storage_t)) / (sizeof(item_t) + sizeof(ntree32::nnode_t) + average_string_len));
            }

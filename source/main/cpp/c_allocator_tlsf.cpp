...
#define tlsf_static_assert(exp) typedef char _tlsf_glue(static_assert, __LINE__)[(exp) ? 1 : -1]
        ...
        */
        ...
        static int block_is_last(const block_header_t* block) { return 0 == block_size(block); }
        ...
        static block_header_t* block_prev(const block_header_t* block) { return block->prev_phys_block; }
...
#undef tlsf_insist
        ...
        ** sentinel block.
        */
        size_t tlsf_pool_overhead() { return 2 * block_header_overhead; }

        size_t tlsf_alloc_overhead() { return block_header_overhead; }
        ...
        pool_t tlsf_add_pool(tlsf_t tlsf, void* mem, size_t bytes)
        {
            ...
            */
            block = offset_to_block(mem, 0 - (tlsfptr_t)block_header_overhead);
...
#    endif
    ...
    void tlsf_alloc_t::init(void* mem, int_t mem_size) { mPool = ntlsf::tlsf_create_with_pool(mem, mem_size); }

    void* tlsf_alloc_t::v_allocate(u32 size, u32 alignment)
    {
        if (alignment <= 8)
            return ntlsf::tlsf_malloc(mPool, size);
        return ntlsf::tlsf_memalign(mPool, alignment, size);
    }

    void tlsf_alloc_t::v_deallocate(void* ptr)
    {
        if (ptr != nullptr)
            ntlsf::tlsf_free(mPool, ptr);
    }

...
#define TLSF_MAX_SIZE (((size_t)1 << (_TLSF_FL_MAX - 1)) - sizeof(size_t))
        ...
        public:
            ...
            const char* check();
#else
            const char* check() { return nullptr; }
#endif

            virtual void* v_resize(size_t) = 0;
        };
...
#    define INLINE static inline __attribute__((always_inline))
        ...
        INLINE size_t block_size(const tlsf_block_t* block) { return block->header & ~BLOCK_BITS; }
        ...
        INLINE bool block_is_free(const tlsf_block_t* block) { return !!(block->header & BLOCK_BIT_FREE); }

        INLINE bool block_is_prev_free(const tlsf_block_t* block) { return !!(block->header & BLOCK_BIT_PREV_FREE); }

        INLINE void block_set_prev_free(tlsf_block_t* block, bool free) { block->header = free ? block->header | BLOCK_BIT_PREV_FREE : block->header & ~BLOCK_BIT_PREV_FREE; }
        ...
        INLINE char* align_ptr(char* p, size_t align) { return (char*)align_up((size_t)p, align); }

        INLINE char* block_payload(tlsf_block_t* block) { return (char*)block + D_OFFSET_OF(tlsf_block_t, header) + BLOCK_OVERHEAD; }
        ...
        INLINE tlsf_block_t* block_from_payload(void* ptr) { return to_block((char*)ptr - D_OFFSET_OF(tlsf_block_t, header) - BLOCK_OVERHEAD); }
        ...
        INLINE bool block_can_split(tlsf_block_t* block, size_t size) { return block_size(block) >= sizeof(tlsf_block_t) + size; }
        ...
        INLINE void remove_free_block(tlsf_t* t, tlsf_block_t* block, uint32_t fl, uint32_t sl)
        {
            ASSERTS(fl < FL_COUNT, "wrong first level");
            ASSERTS(sl < SL_COUNT, "wrong second level");
        ...
        INLINE void insert_free_block(tlsf_t* t, tlsf_block_t* block, uint32_t fl, uint32_t sl)
        {
        ...
        INLINE tlsf_block_t* block_ltrim_free(tlsf_t* t, tlsf_block_t* block, size_t size)
        {
            ASSERTS(block_is_free(block), "block must be free");
        ...
        void* tlsf_t::malloc(size_t size)
        {
            size = adjust_size(size, ALIGN_SIZE);
            if (UNLIKELY(size > TLSF_MAX_SIZE))
                return NULL;
            tlsf_block_t* block = block_find_free(this, size);
            if (UNLIKELY(!block))
                return NULL;
            return block_use(this, block, size);
        }

        void* tlsf_t::aalloc(size_t align, size_t size)
        {
            tlsf_t* t      = this;
            size_t  adjust = adjust_size(size, ALIGN_SIZE);

            if (UNLIKELY(!size || ((align | size) & (align - 1)) /* align!=2**x, size!=n*align */ || adjust > TLSF_MAX_SIZE - align - sizeof(tlsf_block_t) /* size is too large */))
                return NULL;
        ...
        void tlsf_t::free(void* mem)
        {
            ...
            block = block_merge_prev(this, block);
            block = block_merge_next(this, block);

            if (UNLIKELY(!block_size(block_next(block))))
                arena_shrink(this, block);
            else
                block_insert(this, block);
        }

        void* tlsf_t::realloc(void* mem, size_t size)
        {
            tlsf_t* t = this;
    ...
    class tlsf_alloc_t : public alloc_t, public ntlsf::tlsf_t
    {
    public:
        void* mMemory;
        int_t mMemorySize;
        ...
        virtual void* v_resize(ntlsf::size_t) final;
    ...
    tlsf_alloc_t::tlsf_alloc_t() : mMemory(nullptr), mMemorySize(0) {}
    tlsf_alloc_t::~tlsf_alloc_t() {}

    void* tlsf_alloc_t::v_resize(ntlsf::size_t size)
    {
        if (size > mMemorySize)
            return nullptr;
        return mMemory;
    ...
    void* tlsf_alloc_t::v_allocate(u32 size, u32 alignment)
    {
        if (alignment < 16)
            return malloc(size);
        return aalloc(alignment, size);
    }

    void tlsf_alloc_t::v_deallocate(void* ptr) { free(ptr); }

    alloc_t* g_create_tlsf(void* mem, int_t mem_size)
    {
        tlsf_alloc_t* alloc = new (mem) tlsf_alloc_t();
        alloc->mMemory      = nmem::ptr_align( (u8*)mem + sizeof(tlsf_alloc_t), 16);

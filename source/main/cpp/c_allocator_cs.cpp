...
namespace ncore
{
    namespace ncs
    {
        ...
        static u32 s_instance_index(object_t const* object, u16 const component_type_index, void const* component_ptr)
        {
            component_type_t const& cptype      = object->m_a_component[component_type_index];
        ...
        static void s_destroy_instance(object_t* object, u32 instance_index)
        {
        ...
        static bool s_has_cp(object_t* object, u32 instance_index, u16 cp_index)
        {
        ...
        {
            s32 const instance_index = s_create_instance(m_object);
            return s_add_cp(m_object, instance_index, cp_index);
        }
        ...
        void allocator_t::destroy_instance(u16 cp1_index, void* cp1)
        {
            ...
            if (cp1 != nullptr)
            {
                ...
                s_destroy_instance(m_object, instance_index);
            }
        }
        ...
        bool allocator_t::has_cp(u16 cp1_index, void const* cp1, u16 cp2_index) const
        {
            ...
            u32 const instance_index = s_instance_index(m_object, cp1_index, cp1);
            return s_has_cp(m_object, instance_index, cp2_index);
        }
        ...
        void* allocator_t::add_cp(u16 cp1_index, void const* cp1, u16 cp2_index)
        {
            ASSERT(is_component_registered(cp1_index) && is_component_registered(cp2_index));
            u32 const instance_index = s_instance_index(m_object, cp1_index, cp1);
            return s_add_cp(m_object, instance_index, cp2_index);
        }
        ...
        void* allocator_t::rem_cp(u16 cp1_index, void const* cp1, u16 cp2_index)
        {
            ASSERT(is_component_registered(cp1_index) && is_component_registered(cp2_index));
            u32 const instance_index = s_instance_index(m_object, cp1_index, cp1);
            return s_rem_cp(m_object, instance_index, cp2_index);
        }
        ...
        bool allocator_t::has_tag(u16 cp_index, void const* cp_ptr, u8 tg_index) const
        {
            if (cp_ptr != nullptr)
            {
                ...
                u32 const instance_index = s_instance_index(m_object, cp_index, cp_ptr);
                return !s_has_tag(m_object, instance_index, tg_index);
            }
        ...
        void allocator_t::add_tag(u16 cp_index, void const* cp_ptr, u8 tg_index)
        {
            if (cp_ptr != nullptr)
            {
                ASSERT(is_component_registered(cp_index));
                u32 const instance_index = s_instance_index(m_object, cp_index, cp_ptr);
                s_add_tag(m_object, instance_index, tg_index);
            }
        }
        ...
        void allocator_t::rem_tag(u16 cp_index, void const* cp_ptr, u8 tg_index)
        {
            if (cp_ptr != nullptr)
            {
                ASSERT(is_component_registered(cp_index));
                u32 const instance_index = s_instance_index(m_object, cp_index, cp_ptr);
                s_rem_tag(m_object, instance_index, tg_index);
            }
        }
        ...
        void* allocator_t::iterate_begin(u16 cp_index) const
        {
            ...
            {
                ASSERT(is_component_registered(cp_index));
                return s_get_cp(m_object, instance_index, cp_index);
            }
            return nullptr;
        }
